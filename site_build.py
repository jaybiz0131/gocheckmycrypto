...
SLOGAN = "The honest voice in a shill-filled space."
DESC = ("Crypto Cronkite is an honest crypto news desk. AI does the reading, triage, and "
        "fact-checking; a human editor signs off on every story. We report events, we never "
        "advise trades.")
...
def footer():
  ...
  <p class="fnote"><b>{esc(NFA)}</b> Crypto Cronkite is an independent news desk. Stories are
    machine-assembled and machine-verified, then reviewed and approved by a human editor before
    publication. Sources are linked on every story. &copy; {YEAR} {esc(NAME)}.</p>
</div></footer>"""
...
def render_index(items, dateline):
    ...
    <p class="dek">{esc(SLOGAN)} The first published brief lands here. In the meantime, read how
...
def render_about(dateline):
  ...
  <p>Crypto Cronkite is built on one idea: report the real news, strip the shill, and never tell you
     ...
     straight with people. That is the register we hold ourselves to.</p>
  ...
  <p class="nfa">{esc(NFA)}</p>
    ...
    return shell(f"About - {NAME}", "Why Crypto Cronkite exists: an honest crypto news desk.",
                 "About", body, dateline)
