...
a story ONLY when every gate passes:
  ...
  4. If the story turns on a version, date range or threshold, the who-is-affected boundary
     is confirmed verbatim against the vendor's or agency's OWN advisory (see boundary.py).
     An unconfirmed boundary holds the story with no retry path.
...
def run(approval_path=None, force_dry=False):
    ...
    if mode != "live":
        ...
        run_mode = os.environ.get("CRYPTO_LLM_MODE", "live")
        ...
        common.gh("notice" if expected else "error",
                  ...
                  + ("" if expected else f" NOTE: this run is CRYPTO_LLM_MODE={run_mode}; a live "
                                         f"run must not carry a {mode} approval file."))
