...
def check(client, obj, stories, boards, extras=None, edition_date=None):
    ...
    for p in v["problems"]:
        ...
        # A SELF-REFUTING 'ABSENT' DIES HERE TOO (2026-08-27): runs 165 and 896
        # each killed a good edition on 'absent' items whose own reasoning ENDED
        # "The edition's claims match the data exactly" / "confirmed correct". Same
    ...
    def _date_legend(excerpts):
        ...
        weekdays: run 165 killed an edition for calling 2026-08-27 'Thursday'
        because the checker believed that date was a Wednesday (it is a Thursday),
        and the calibration explicitly protects weekday references consistent with
        an input dateline. Deterministic, authoritative, and only for dates the
        excerpts actually carry."""
...
def build_item(edition, obj, stories, date, published_utc):
    ...
    return {
        ...
        "human_take": "",
        "body": paras,
        # a quiet-day (boards-only) edition cites what it actually read: the desk's
        # own market boards. An edition with no citation at all fails the canary, and
        # should: every edition names its inputs (owner directive 2026-08-25).
...
def main():
    ...
    if not stories and not boards:
        # nothing to say AND nothing to show: silent is honest, but it is now LOUD
        # silent (owner directive 2026-08-25; the 42h dark spell was invisible because
        # this was a bare print)
        common.gh("warning", "wrap: zero stories in the window and no boards; the "
                             "edition abstains. edition_check counts the gap and fails "
                             "the run once it breaches.")
        common.write_out("wrap-status.json", {"edition": edition, "date": date,
                                              "mode": "abstain",
                                              "reasons": ["no stories and no boards"]})
        return 0
    if not stories:
        # BOARDS-ONLY QUIET-DAY EDITION (owner directive 2026-08-25, and the desk's own
        # doctrine all along: never-empty-slot, and prompts/wrap.md licenses a short
        # honest quiet day). The boards are fresh every run; a market desk with zero
        # publishable stories still has a market to read. The old code checked stories
        # BEFORE fetching boards, so the boards-only edition its own docstring promised
        # was structurally unreachable, and the desk went dark for 42 hours behind
        # green runs while the writer was refusing unreadable sources.
        common.gh("notice", "wrap: zero stories in the window; writing the boards-only "
                            "quiet-day edition the doctrine calls for")
    ...
    def _trace(o):
        if client.mode != "live":
            return True, [], []
        ok_, reasons_ = check(client, o, stories, boards or {},
